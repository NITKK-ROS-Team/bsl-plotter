...
namespace bsl_plotter_example
{
    ...
    private:
        void send_goal()
        {
            ...
            goal_msg.trajectory.joint_names = {"rev1", "rev2"};
            goal_msg.trajectory.points.resize(1);
            double rev1 = get_random(0.0,3.14);
            double rev2 = get_random(1.4,2.0);
            RCLCPP_INFO(this->get_logger(), "rev1: %f, rev2: %f", rev1, rev2);
            goal_msg.trajectory.points[0].positions = {rev1, rev2};
        ...
        {
            RCLCPP_DEBUG(
                this->get_logger(), "Next waypoint: %s %f",
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<bsl_plotter_example::RandomMove>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
